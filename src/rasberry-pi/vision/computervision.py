...
Complete autonomous driving pipeline: Vision + State Machine + Serial
...
class BossModeVision:
    ...
    def __init__(self, resolution=(320, 240), framerate=15, driving_direction=1):
        ...
        self.camera.start()
        time.sleep(0.5)
        logging.info(f"Vision initialised (Camera Module 3). Direction: {'CW' if driving_direction == 1 else 'CCW'}")
...
class SerialDataHandler:
    ...
    def send_command(self, steering, speed, state):
        """
        Send command to Arduino (thread-safe).
        Format: "steering,speed,state\n"
        Arduino parses only the first three values.
        State must be one of: STRAIGHT, CORNER, OBSTACLE, REVERSE, PARKING, LEAVING_PARKING
        """
        if not self.connected or not self.ser:
            return False
    ...
    def send_challenge(self, challenge_type):
        """
        Send challenge type as a one-time message.
        Format: "OPEN" or "OBSTACLE" (no prefix, no extra commas)
        Arduino expects exactly these strings.
        """
        if not self.connected or not self.ser:
            return False

        challenge_type = challenge_type.upper()
        if challenge_type not in ["OPEN", "OBSTACLE"]:
            print(f"[ERROR] Invalid challenge type: {challenge_type}")
            return False

        command = f"{challenge_type}\n"
        with self.lock:
            try:
                self.ser.write(command.encode())
                return True
            except Exception as e:
                print(f"[ERROR] Challenge send failed: {e}")
                return False
...
class RobotState:
    ...
    def determine_challenge_type(self, vision_result):
        ...
        if vision_result.magenta_detected:
            ...
            print("[INFO] Obstacle Challenge detected")
        else:
            self.challenge_type = "OPEN"
            self.started_in_parking = False
            print("[INFO] Open Challenge detected")
    ...
    def detect_driving_direction(self, vision_result):
        ...
        if vision_result.corner_detected:
            ...
            print(f"[INFO] Direction: {'CW' if self.driving_direction == 1 else 'CCW'}")
            return True
        return False
    ...
    def update(self, vision_result, front_dist, left_dist, right_dist):
        ...
        # SAFE_HOLD recovery
        if (self.state == "OBSTACLE" and self.pillar_phase == "PASSING" and
            front_dist < ESTOP_NORMAL and front_dist >= ESTOP_OBSTACLE and
        ...
        if (front_dist <= 5.0 and (left_dist <= 5.0 or right_dist <= 5.0)
            ...
            self.steering_pid.reset()
            return 0.9, 0.2, "LEAVING_PARKING"
        ...
        if (vision_result.corner_detected and
            ...
            if self.laps >= self.max_laps:
                ...
                self.steering_pid.reset()
                return 0.0, 0.1, "FINISHING"
            self.steering_pid.reset()
            return steering, 0.25, "CORNER"
        ...
        if (front_dist >= 20.0 and
            ...
            vision_result.traffic_sign_colour is None):
            self.state = "STRAIGHT"
            pid_steering = self.steering_pid.update(0.0, vision_result.lane_offset_normalized)
...
def main():
    ...
    # Initialise vision
    vision = BossModeVision(driving_direction=1)
    ...
    with vision:
        ...
        serial_handler.send_challenge(challenge)
        print(f"[INFO] Sent challenge: {challenge}")

        # Wait for Arduino ACK (non-blocking)
        time.sleep(0.1)
        if serial_handler.ser and serial_handler.ser.in_waiting > 0:
            ack = serial_handler.ser.readline().decode().strip()
            if ack.startswith("ACK:"):
                print(f"[INFO] Arduino ACK: {ack}")

        # Pre-seed challenge type
        robot.challenge_determined = True
        robot.challenge_type = challenge
        robot.started_in_parking = (challenge == "OBSTACLE")

        print("[INFO] Control loop started. Press Ctrl+C to stop.")
        print(f"[INFO] PID: Kp={PID_KP}, Ki={PID_KI}, Kd={PID_KD}")
        ...
        try:
            while True:
                ...
                steering, speed, state = robot.update(
                    result, front_dist, left_dist, right_dist
                )

                serial_handler.send_command(steering, speed, state)
                ...
                if frame_time > CONTROL_LOOP_DT * 1.5:
                    print(f"[WARN] Frame {frame_time*1000:.1f}ms (target {CONTROL_LOOP_DT*1000:.0f}ms)")

        except KeyboardInterrupt:
            print("\n[INFO] Stopped by user")
        finally:
            serial_handler.send_command(0.0, 0.0, "STOP")
            serial_handler.stop()
